...
import com.nuaa.app.PamgEnStandar;
import com.nuaa.sys.util.DbUtil;
import com.nuaa.sys.util.IArrayPreparedStatementProcessor;
import com.nuaa.sys.util.IResultSetProcessor;
import com.nuaa.sys.util.Logger;

public class PamgEnStandarImp implements PamgEnStandar {
	...
	public String addFile(HashMap addMap) {
		...
		System.out.println("...................."+indi_num+"....................");
					...
					System.out.println(pa[9]);
					 String[] sqls = new String[1];
						sqls[0] = "insert into  t_mt_entry_trecord (type_id,in_time,newstatus,indi_num,location,keeper,remark,entry_num,indi_id,status) values(?,to_date(?,'yyyy/mm/dd'),?,?,?,?,?,?,?,?)";
						Logger.debug(sqls[0]);
						try {
					...
					sqls[0] = "insert into  t_mt_entry_record (entry_num,indi_id,type_id,in_time,newstatus) values(?,?,?,to_date(?,'yyyy/mm/dd'),?)";
					Logger.debug(sqls[0]);
					try {
								DbUtil.executeBatchs(sqls,
										new IArrayPreparedStatementProcessor() {
											public void process(PreparedStatement[] pstmts)
													throws SQLException {
												pstmts[0].setString(1, pa[7]);
												pstmts[0].setString(2, pa[8]);
					...
					sql[0] = "insert into  t_mt_indi (indi_id,type_id,indi_num,newstatus,location,keeper,remark,status) values (?,?,?,?,?,?,?,?)";
							try {
								DbUtil.executeBatchs(sql,
										new IArrayPreparedStatementProcessor() {
											public void process(PreparedStatement[] pstmts)
													throws SQLException {
												pstmts[0].setString(1, pa[8]);
	...
	public JSONObject getQueryFile(HashMap queryMap) {
		...
		sql="select * from  v_tmtype2com where mnum='"+filter+"'";			
	...
	public boolean chexFile(HashMap chexMap) {
		...
		String sql="select * from  t_mt_type where mnum='"+mnum+"'";
	...
	public String chexTool(HashMap chexToolMap) {
		...
		System.out.println(indi_num);
		String sq="select * from (select * from  t_mt_entry_trecord where indi_num='"+indi_num+"') where status = '批准' or status = '申请'";
		...
		if(result.equals("false")){
			String sql="select * from  t_mt_indi where indi_num='"+indi_num+"'";
			final int []flag=new int[1];
			flag[0]=0;
			try {			
				DbUtil.execute(sql,new IResultSetProcessor() {
	...
	public JSONObject getQuerycomAll(HashMap comMap) {
		...
		String sql="";
		sql="select distinct company from (select * from (select * from v_tmtype2com  where mnum = '"+filter+"')where hq = '"+hq+"')";
	...
	public JSONObject getQuerynumAll(HashMap numMap) {
		...
		String sql="";
		sql="select * from (select * from (select * from (select * from v_tmtype2com)  where mnum = '"+filter+"')where hq = '"+filter2+"')where company = '"+filter1+"'";
	...
	public boolean chexFile1(HashMap chexMap1) {
		...
		String sql="select * from(select * from  t_mt_type where mnum='"+mnum+"')where hq='"+hq+"'";
		final int []flag=new int[1];
		flag[0]=0;
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
	...
	public JSONObject getQueryFile1(HashMap queryMap1) {
		...
		sql="select * from(select * from  v_tmtype2com where mnum='"+filter+"') where hq='"+hq+"'";			
		System.out.println("sql="+sql);
		try {		
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
	...
	public String addFileApp(HashMap addMap) {
		...
		sql[0] = "insert into  t_mt_indi (indi_id,type_id,indi_num,newstatus,location,keeper,remark,status) values (?,?,?,?,?,?,?,?)";
				...
				if (result.equals("truetrue")){
					...
					sqls[0] = "update t_mt_entry_trecord set status=? where entry_num = '"+pa[7]+"'";
					Logger.debug(sqls[0]);
					try {
								DbUtil.executeBatchs(sqls,
										new IArrayPreparedStatementProcessor() {
					...
					sqls[0]="delete from t_mt_entry_trecord where entry_num in('"+pa[7]+"')";
					Logger.debug(sqls[0]);
						try {
							DbUtil.executeBatchs(sqls,
								new IArrayPreparedStatementProcessor() {
	...
	public JSONObject getQueryApprove(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from t_mt_etr_type where status = '批准' or status = '申请'or status = '拒绝' ";
		Logger.debug(sql);
		...
		sql="select * from (select  *  from  (select * from (select * from  t_mt_etr_type ) where status = '批准'or status = '申请' or status = '拒绝' order by status) where rownum<="+(start+limit)+" minus select  *  from  (select  *  from  (select * from  t_mt_etr_type ) where status = '批准'or status = '申请' or status = '拒绝' order by status ) where rownum<="+start+") order by status ";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
	...
	public String deleteRe(HashMap delMap) {
		...
		String []sqls=new String[1];
		sqls[0]="delete from t_mt_entry_trecord where entry_num in('"+deleteid+"')";
		Logger.debug(sqls[0]);
			try {
				DbUtil.executeBatchs(sqls,
					new IArrayPreparedStatementProcessor() {
