Ext.onReady(function(){
		...
		listeners:{
	        select:function(){ 
	        	...
	        	var load=Ext.Ajax.request({
	        		...
	        		method:'post',
	        		params:{type:'VIEWONE',numn:yclassid},//在logic_circuit_board.jsp中定义VIEWONE传递numn参数
	        		success:successFm,
	        		failure:function(){
	        			alert('服务器出现错误请稍后再试！');
	        			}
	...
	function successFm(result,request){
		...
		document.getElementById('yuhong').value =bb;
	}
	...
	function tongji(){
		...
		for(var i=0;i<record.length;i++){
			var a=record[i].get("man_hour");
			sum=sum+a;
		}
		document.getElementById('zhuangpei').value=sum/60;
	...
	function jisuan(){
		...
		url:'logic_circuit_board.jsp',
		method:'post',
		params:{type:'VIEWONE',numn:editnumn},//传递numn参数
		success:successFnn,
		failure:function(){
			alert('服务器出现错误请稍后再试！');
			}
	...
	function successFnn(result,request){
		var objtp=Ext.util.JSON.decode(result.responseText);
		document.getElementById('gongshi').value =parseInt(objtp.work_time);
