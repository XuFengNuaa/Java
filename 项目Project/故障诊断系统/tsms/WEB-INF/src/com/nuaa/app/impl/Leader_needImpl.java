...
import com.nuaa.app.Leader_need;
import com.nuaa.sys.util.DbUtil;
import com.nuaa.sys.util.IArrayPreparedStatementProcessor;
import com.nuaa.sys.util.IResultSetProcessor;
import com.nuaa.sys.util.Logger;

public class Leader_needImpl implements Leader_need {
...
//	查询刀具需求申请
	public JSONObject getQuerycut(HashMap queryMap) {
		...
		String sql="";
		sql="select count(*) as ct from v_cut2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		...
		sql="select * from v_cut2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询非测量工具需求申请
	public JSONObject getQueryct(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_ct2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_ct2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询量具需求申请
	public JSONObject getQuerym(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_m2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_m2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询测量工具需求申请
	public JSONObject getQuerymt(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_mt2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_mt2need where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	判断特殊借出时同一时间段内这些待审批刀具是否被处理
	public boolean checkCut(HashMap checkHMap) {
		...
		sql="select * from v_cut2need where trim(leader_approve)='未处理' and need_num in("+need_nums+")";  
		Logger.debug(sql);
...
//	判断特殊借出时同一时间段内这些待审批测量工具是否被处理
	public boolean checkCt(HashMap checkHMap) {
		...
		String counts=(String)checkHMap.get("count");
		final int Count = Integer.parseInt(counts);
		sql="select * from v_ct2need where trim(leader_approve)='未处理' and need_num in("+need_nums+")";  
		Logger.debug(sql);
		final int[] count=new int[1];
		count[0]=1;
		try {			
...
//	判断特殊借出时同一时间段内这些待审批量具是否被处理
	public boolean checkM(HashMap checkHMap) {
		...
		String counts=(String)checkHMap.get("count");
		final int Count = Integer.parseInt(counts);
		sql="select * from v_m2need where trim(leader_approve)='未处理' and need_num in("+need_nums+")";  
		Logger.debug(sql);
		final int[] count=new int[1];
		count[0]=1;
		try {			
...
//	判断特殊借出时同一时间段内这些待审批测量工具是否被处理
	public boolean checkMt(HashMap checkHMap) {
		...
		String counts=(String)checkHMap.get("count");
		final int Count = Integer.parseInt(counts);
		sql="select * from v_mt2need where trim(leader_approve)='未处理' and need_num in("+need_nums+")";  
		Logger.debug(sql);
		final int[] count=new int[1];
		count[0]=1;
		try {			
...
//	刀具需求批准
	public boolean appCut(HashMap appcutHMap) {
		...
		try {
			sqls[0]="update t_c_need set leader_approve=?,leader_time=?,leader_num=? where need_num in("+need_nums+")";
...
//	非测量工具需求批准
	public boolean appCt(HashMap appcutHMap) {
		...
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_ct_need set leader_approve=?,leader_time=?,leader_num=? where need_num in("+need_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"批准");
							pstmts[0].setDate(2,curDate);
...
//	量具需求批准
	public boolean appMmt(HashMap appmmtHMap) {
		...
		final String leader_num=(String)appmmtHMap.get("leader_num");
		Calendar c = Calendar.getInstance();
		final java.sql.Date curDate= new java.sql.Date(c.getTime().getTime());
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_m_need set leader_approve=?,leader_time=?,leader_num=? where need_num in("+need_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"批准");
							pstmts[0].setDate(2,curDate);
...
//	测量工具需求批准
	public boolean appMt(HashMap appmmtHMap) {
		...
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_mt_need set leader_approve=?,leader_time=?,leader_num=? where need_num in("+need_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"批准");
							pstmts[0].setDate(2,curDate);
...
//	拒绝刀具需求
	public boolean refCut(HashMap refcutHMap) {
		...
		final String leader_num=(String)refcutHMap.get("leader_num");
		Calendar c = Calendar.getInstance();
		final java.sql.Date curDate= new java.sql.Date(c.getTime().getTime());
		String[] sqls=new String[1];
		try {
...
//	拒绝量具需求
	public boolean refMmt(HashMap refmmtHMap) {
		...
		final String leader_num=(String)refmmtHMap.get("leader_num");
		Calendar c = Calendar.getInstance();
		final java.sql.Date curDate= new java.sql.Date(c.getTime().getTime());
		String[] sqls=new String[1];
		try {
