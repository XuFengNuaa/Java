Ext.onReady(function(){
    ...
    var record_english= Ext.data.Record.create([    		
    		{name:'tpmc'},
    		{name:'time'},
    		{name:'id'}	
    ]); 
    ...
    //调用数据库信息
	var hqObj={url:'logic_english.jsp',method: 'POST',extraParams:{type:'QUERYCH',params:{start:start,limit:limit},filter:filter}};
    ...
    ds.load(obj);
    var hqObjAni={url:'logic_english.jsp',method:'POST',extraParams:{}};
	var record_english=new Ext.data.Store({
					...
					fn:function(btn){					              
						if (btn=='ok'){	
						   Ext.showProgressDialog();							
							...
							Ext.Ajax.request({
						   	url: 'logic_english.jsp',
						   	params: {type:'DEL',id:id1},
	...
	function getDialog1(){	
		...
		if (!dialog1) {
				dialog1 = new Ext.ux.UploadDialog.Dialog({
				  url: 'logic_english.jsp?type=uploadimg&special='+special,
				  reset_on_hide: false,
				  modal:true,
				  closable : false,   
		          collapsible : false,   
