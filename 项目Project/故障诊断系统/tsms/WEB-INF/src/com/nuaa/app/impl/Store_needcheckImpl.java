...
import com.nuaa.app.Store_needcheck;
import com.nuaa.sys.util.DbUtil;
import com.nuaa.sys.util.IArrayPreparedStatementProcessor;
import com.nuaa.sys.util.IResultSetProcessor;
import com.nuaa.sys.util.Logger;

public class Store_needcheckImpl implements Store_needcheck {
...
//	查询刀具需求申请
	public JSONObject getQuerycut(HashMap queryMap) {
		...
		String sql="";
		sql="select count(*) as ct from v_cut2need where trim(status)='未处理' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		...
		sql="select * from v_cut2need where trim(status)='未处理'";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询非测量工具需求申请
	public JSONObject getQueryct(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_ct2need where trim(status)='未处理' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_ct2need where trim(status)='未处理'";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询量具需求申请
	public JSONObject getQuerym(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_m2need where trim(status)='未处理' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_m2need where trim(status)='未处理'";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询测量工具需求申请
	public JSONObject getQuerymt(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_mt2need where trim(status)='未处理' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_mt2need where trim(status)='未处理'";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	刀具需求批准
	public boolean appCut(HashMap appcutHMap) {
		...
		try {
			sqls[0]="update t_c_need set status=? where need_num in("+need_nums+")";
...
//	非测量工具需求批准
	public boolean appCt(HashMap appcutHMap) {
		...
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_ct_need set status=? where need_num in("+need_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"已处理");
							pstmts[0].addBatch();
...
//	量具需求批准
	public boolean appMmt(HashMap appmmtHMap) {
		...
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_m_need set status=? where need_num in("+need_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"已处理");
							pstmts[0].addBatch();
...
//	测量工具需求批准
	public boolean appMt(HashMap appmmtHMap) {
		...
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_mt_need set status=? where need_num in("+need_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"已处理");
							pstmts[0].addBatch();
