...
import com.nuaa.app.ShouJuanMod;
...
public class ShouJuanModImpl implements ShouJuanMod{
	...
	public boolean addFile(HashMap addMap) {
		...
		System.out.println(pa[1]);
		 sql[0] = "insert into t_shoujuan (ID,dalei_sim,xiaolei_sim,machine_sim,material_sim,daihao_sim,mingcheng_sim,tuhao_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		 try {
	...
	public JSONObject getQueryFile(HashMap hashmap) {
		...
		String sql="";
		sql="select  count(*) as ct  from t_shoujuan ";
		...
		String sql1 = "select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				...
				"(select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				"t_shoujuan where "+filter+" order by "+order+") where rownum<="+(start+limit)+" minus select  " +
				...
				"(select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				"t_shoujuan where "+filter+" order by "+order+" ) where rownum<="+start+") order by "+order; 
		try {
	...
	public 	String getFile(String dir,String tuhao){
		...
		File fii=new File(dir+"\\web\\tempfile1\\"+tuhao+"\\");
		System.out.println(dir+"\\web\\tempfile1\\"+tuhao+"\\");
		if(!fii.exists()) {
		...
		for(int i=0;i<files.length;i++){
			...
			if(typ.equals("jpg")||typ.equals("png")||typ.equals("txt")||typ.equals("mp4")||typ.equals("mp3")){
				bb += "<a href=\"..\\..\\..\\..\\..\\tsms\\web\\tempfile1\\"+tuhao+"\\" +
	...
	public HashMap uploadFile1(HashMap fileHmap) {
    	...
    	String tempPath = "\\web\\tempfile1\\"+id+"\\"; // 用于存放临时文件的目录
    	//String imgdir = "\\image\\";
		String trace="";
		String filename="";
		String fileid="";
	...
	public String delPart(HashMap hashMap) {
		...
		sql[0] = "delete from t_shoujuan where id in ('',"+id+")";
		String sql2 = "select * from t_shoujuan where id in ('',"+id+")";
		try {
