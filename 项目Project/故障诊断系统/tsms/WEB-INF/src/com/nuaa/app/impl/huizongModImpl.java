...
import com.nuaa.app.huizongMod;
...
public class huizongModImpl implements huizongMod{
	  public String add_cutter(HashMap add_cutter_HMap) {
			...
			String sql ="";	
			String people=(String)add_cutter_HMap.get("people");
			String in_date=(String)add_cutter_HMap.get("date");
			String pclass=(String)add_cutter_HMap.get("pclass");
			String bclass=(String)add_cutter_HMap.get("bclass");
			String problem=(String)add_cutter_HMap.get("problem");
			String reason=(String)add_cutter_HMap.get("reason");
			String solution=(String)add_cutter_HMap.get("solution");
			String remark=(String)add_cutter_HMap.get("remark");
			
			Logger.debug(sql);			
			final String[] count=new String[2];
			
			sql="select count(*) as ct from T_huizong where pclass='"+pclass+"'  ";			
			try {			
			...
			try {
				//if ("0".equals(count[0])){					
					...
					sqls[0]= "insert into T_huizong(id,people,in_date,pclass,bclass,problem,reason,solution,remark) values(?,?,to_date(?,'yyyy/mm/dd'),?,?,?,?,?,?)";
					DbUtil.executeBatchs(sqls,
							...
							if(add_cutterHashMap.get("people")!=null){
								pstmts[0].setString(2,(String)add_cutterHashMap.get("people"));//people
							}else{
								pstmts[0].setString(2,"");//mnum
							}
							if(add_cutterHashMap.get("date")!=null){
								pstmts[0].setString(3,(String)add_cutterHashMap.get("date"));//class
							}else{
							...
							if(add_cutterHashMap.get("problem")!=null){
								pstmts[0].setString(6,(String)add_cutterHashMap.get("problem"));//problem
							}else{
								pstmts[0].setString(6,"");//diviv
							}
							if(add_cutterHashMap.get("reason")!=null){
								pstmts[0].setString(7,(String)add_cutterHashMap.get("reason"));//companyid
							}else{
								pstmts[0].setString(7,"");//company
							}
							if(add_cutterHashMap.get("solution")!=null){
								pstmts[0].setString(8,(String)add_cutterHashMap.get("solution"));//mini_qs
							}else{
								pstmts[0].setString(8,"");//mini_qs
							}
							if(add_cutterHashMap.get("remark")!=null){
								pstmts[0].setString(9,(String)add_cutterHashMap.get("remark"));//hq
							}else{
	  ...
	  public JSONObject getQuery_cutter(HashMap query_cutterMap) {
		  ...
		  //V_CUTTER2COM是t_cutter的视图，类似于继承过来
			...
			final String filter = (String)query_cutterHashMap.get("filter");
			final String pclass = (String)query_cutterHashMap.get("pclass");
			final String bclass = (String)query_cutterHashMap.get("bclass");
			final JSONObject jsonObj=new JSONObject();
			final JSONArray jsonArray=new JSONArray();
			String sql="";
			sql="select  count(*) as ct  from  T_huizong where " + filter;
		...
		//	int limit=Integer.parseInt((String)query_cutterHashMap.get("limit"));
			...
			sql="select * from (select * from  (select * from T_huizong where "+pclass+")where "+bclass+")where "+filter ; 
					...
					public void process(ResultSet rs) throws SQLException {					
						try {
							...
							//	decimalformat.applyPattern("###.##");
								...
								obj.put("solution", rs.getString("solution"));
								obj.put("people",rs.getString("people"));
								obj.put("date", rs.getString("in_date"));	
		...
		public String edit_cutter(HashMap edit_cutterHMap) {
			...
			final String people=(String)edit_cutterHMap.get("people");
			final String pclass=(String)edit_cutterHMap.get("pclass");
			final String bclass=(String)edit_cutterHMap.get("bclass");
			final String in_date=(String)edit_cutterHMap.get("date");
			final String problem=(String)edit_cutterHMap.get("problem");
			final String reason=(String)edit_cutterHMap.get("reason");
			final String remark=(String)edit_cutterHMap.get("remark");
			final String solution=(String)edit_cutterHMap.get("solution");
			String result = "true";
			String sql ="";	
			final String[] count=new String[1];
            sql="select count(*) as ct from T_huizong where id='"+id+"'";
            Logger.debug(sql);
				try {			
					DbUtil.execute(sql,new IResultSetProcessor(){
						public void process(ResultSet rs) throws SQLException{
			...
			try {
				if ("1".equals(count[0])){	
			String[] sqls = new String[1];	
			sqls[0] = "update T_huizong set people=?,pclass=?,bclass=?,problem=?,reason=?,solution=?,remark=?,in_date=to_date(?,'yyyy/mm/dd')  where id='"+id+"'";
			System.out.println(sqls[0]);
				DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor(){
					public void process(PreparedStatement[] pstmts)	throws SQLException {			
						pstmts[0].setString(1,people);
						pstmts[0].setString(2,pclass);
						pstmts[0].setString(3,bclass);
						pstmts[0].setString(4,problem);
						pstmts[0].setString(5,reason);
						pstmts[0].setString(6,solution);
						pstmts[0].setString(7,remark);
						pstmts[0].setString(8,in_date);						
						pstmts[0].addBatch();
					}
				});
				result="true";
		...
		public String del_cutter(HashMap del_HMap){// TODO Auto-generated method stub
			...
			String result = "true";
			sql[0] = "delete from T_huizong where id in ('',"+ id +")";
			Logger.debug(sql[0]);
		...
		public String edit_grind_cutter(HashMap edit_grdcutterHMap) {
			// TODO 自动生成方法存根			
			final String people=(String)edit_grdcutterHMap.get("people");		
			final String pclass=(String)edit_grdcutterHMap.get("pclass");			
			final String bclass=(String)edit_grdcutterHMap.get("bclass");			
			final String in_date=(String)edit_grdcutterHMap.get("date");
			final String problem=(String)edit_grdcutterHMap.get("problem");				
			final String reason=(String)edit_grdcutterHMap.get("reason");		
			final String solution=(String)edit_grdcutterHMap.get("solution");		
			...
			System.out.println(sqls[0]);
				DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor(){
					public void process(PreparedStatement[] pstmts)	throws SQLException {							
						...
						pstmts[0].setString(3,people);						
						pstmts[0].setString(4,in_date);						
						pstmts[0].setString(5,problem);
						pstmts[0].setString(6,reason);
						pstmts[0].setString(7,solution);
						pstmts[0].setString(8,remark);					
						pstmts[0].addBatch();
					}
				});
				return "1";
		...
		public JSONObject view_cutter(HashMap view_cutterHMap) {
			...
			String sql="";
			sql="select * from t_huizong where id='"+id+"'"; 
					...
					public void process(ResultSet rs) throws SQLException {					
						try {
							while(rs.next()){
								...
								jsonObj.put("reason", rs.getString("reason"));								
								jsonObj.put("solution", rs.getString("solution"));
								jsonObj.put("people",rs.getString("people"));
								jsonObj.put("date", rs.getString("in_date"));
