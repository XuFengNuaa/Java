Ext.onReady(function(){
    ...
    //定义查询结果页面
	var hpObj={url:'logicmillingquery.jsp',method: 'POST',extraParams:{}};
...
var panel = new Ext.Panel({
	...
	if(string[2]==''){
	    filter = "mill_date between to_date('"+beginDate+"','yyyy-mm-dd') and to_date('"+endDate+"','yyyy-mm-dd')";
	    }
	else{		
			 
			if(string[0] == 'front'){
				    filter += " and " + string[1] + " like '" + string[2] + "%' "+" and mill_date between to_date('"+beginDate+"','yyyy-mm-dd') and to_date('"+endDate+"','yyyy-mm-dd')";
				  }else if(string[0] == 'exact'){
				    filter += " and " + string[1] + " = '" + string[2] + "' "+" and mill_date between to_date('"+beginDate+"','yyyy-mm-dd') and to_date('"+endDate+"','yyyy-mm-dd')";
    ...
    //good_name,part_name,good_num,part_num,mill_date
	//getOrder()
	function getOrder(){	
	    var orderValue="";
    ...
    var record_detail = Ext.data.Record.create([    		
    	...
    	var hpObj_detail = {url:'logicmillingquery.jsp',method: 'POST',extraParams:{}};
 ...
 //返回函数
	...
	function viewFn(){
		...
		if(record.length==0){
			...
			document.getElementById('milldatedetail').value = record[0].get("milldate").toString();
