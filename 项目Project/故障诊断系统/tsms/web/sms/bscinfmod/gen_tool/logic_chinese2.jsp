...
<%@ page import="java.util.*,org.json.*,java.io.*,com.nuaa.sys.util.DbUtil,org.json.JSONObject, org.json.JSONArray,com.nuaa.sys.util.PublicUtil,java.util.UUID,java.sql.*,com.nuaa.sys.util.AppInsFactory,java.sql.Date,com.nuaa.app.chinese"%>
...
//			   nowtime=UUID.randomUUID().toString();
			...
			result=sd.getQueryApprove2(hashMap).toString();
		}else if("VIEW2".equals(type)){
			if (request.getParameter("tuhao") != null) {
				String tuhao = request.getParameter("tuhao");
				 String dir=request.getRealPath("");
				 result =  sd.getFile2(dir,tuhao);
				
				System.out.println("sassg"+result);
			}//材料                                                          
			
				}else if("DEL2".equals(type)){
			HashMap hashMap=new HashMap();
			if(request.getParameter("id")!=null){
				hashMap.put("id",request.getParameter("id"));
			}
			String returnValue = sd.delPart2(hashMap);
