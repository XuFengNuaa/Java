...
import com.nuaa.app.LeaderApp;
import com.nuaa.sys.util.DbUtil;
import com.nuaa.sys.util.IArrayPreparedStatementProcessor;
import com.nuaa.sys.util.IResultSetProcessor;
import com.nuaa.sys.util.Logger;

public class LeaderAppImpl implements LeaderApp {
	
//	查询已申请且班组长已批准但领导未批准的高质新刀具
	public JSONObject getQuerycut(HashMap queryMap) {
		...
		String sql="";
		sql="select count(*) as ct from v_cut2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准'  and trim(ways) is null and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		...
		sql="select * from v_cut2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways) is null order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
					try {
						while(rs.next()){
							...
							obj.put("gleadername",rs.getString("gleadername"));
...
//	查询已申请且班组长已批准但领导未批准的高质新刀具
	public JSONObject getQuerycutTem(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_cut2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways)='1' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_cut2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways)='1' order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	查询已申请且班组长已批准但领导未批准的高质新非测量工具
	public JSONObject getQueryct(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_ct2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways) is null and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_ct2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways) is null order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	暂借高质新非测量工具批准
	public JSONObject getQueryctTem(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from v_ct2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways)='1' and "+(String)queryHashMap.get("filter");
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from v_ct2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and trim(ways)='1' order by apply_time DESC";  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
...
//	判断特殊借出时同一时间段内这些待审批刀具是否被处理
	public boolean checkCut(HashMap checkHMap) {
		...
		sql="select * from v_cut2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and apply_num in("+apply_nums+")";  
		Logger.debug(sql);
...
//	判断特殊借出时同一时间段内这些待审批测量工具是否被处理
	public boolean checkCt(HashMap checkHMap) {
		...
		String counts=(String)checkHMap.get("count");
		final int Count = Integer.parseInt(counts);
		sql="select * from v_ct2hqapply where trim(leader_approve)='未处理' and trim(gleader_approve)='批准' and apply_num in("+apply_nums+")";  
		Logger.debug(sql);
		final int[] count=new int[1];
		count[0]=1;
		try {			
	...
	public boolean appCut(HashMap appcutHMap) {
		...
		try {
			sqls[0]="update t_hq_c_apply_record set leader_approve=?,leader_time=?,leader_num=? where apply_num in("+apply_nums+")";
	...
	public boolean appCt(HashMap appcutHMap) {
		...
		String[] sqls=new String[1];
		try {
			sqls[0]="update t_hq_ct_apply_record set leader_approve=?,leader_time=?,leader_num=? where apply_num in("+apply_nums+")";
			DbUtil.executeBatchs(sqls,new IArrayPreparedStatementProcessor() {
						public void process(PreparedStatement[] pstmts)throws SQLException {	
							pstmts[0].setString(1,"批准");
							pstmts[0].setDate(2,curDate);
