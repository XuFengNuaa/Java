...
function t(f){window.open(f);};
	...
	var hpObj={url:'logicm_delete.jsp',method: 'POST',extraParams:{}};
	...
	function customQueryFn(){
	    ...
	    var partdalei= Ext.data.Record.create([
		 ...
		 var storedalei = new Ext.data.Store ({
		     proxy: new Ext.data.HttpProxy({url:"logicm_to_check.jsp?type=QUERYDALEI"}), // 数据源
		     reader: new Ext.data.JsonReader({
		         id:'id',
			     root:"filedata"
			       },partdalei),
		...
		var daleicombox=new Ext.form.ComboBox({
			...
			listeners:{
				select:function(){
					storexiaolei.load({params:{dalei:'热轧'}});
				}
				},
			emptyText:'请选择案例大类',
			allowBlank:true,
		...
		var storexiaolei = new Ext.data.Store ({
	     proxy: new Ext.data.HttpProxy({url:"logicm_to_check.jsp?type=QUERYXIAOLEI"}), // 数据源
	     reader: new Ext.data.JsonReader({
	         id:'id',
		     root:"filedata"
		       },partxiaolei),
	  ...
	  // 定义添加窗口材料
		 ...
		 var materialCom = new Ext.data.Store ({
		     proxy: new Ext.data.HttpProxy({url:"logicm_to_check.jsp?type=QUERYMATERIAL"}), // 数据源
		     reader: new Ext.data.JsonReader({
		         id:'id',
			     root:"filedata"
			       },material_com),
		 ...
		 // 定义机床型号
			 ...
			 var machineCom = new Ext.data.Store ({
			     proxy: new Ext.data.HttpProxy({url:"logicm_to_check.jsp?type=QUERYMACHINE"}), // 数据源
			     reader: new Ext.data.JsonReader({
			         id:'id',
				     root:"filedata"
				       },machine_com),
		...
		if (!dialog1) {
				dialog1 = new Ext.ux.UploadDialog.Dialog({
				  url: 'logicm_delete.jsp?type=uploadimg&id='+id1,
				  reset_on_hide: false,
				  modal:true,
				  closable : false,   
		          collapsible : false,   
