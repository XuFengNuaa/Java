...
public class UserXmlImpl implements UserExcel {
	...
	public void createExcelUser(HashMap hashMap, OutputStream os)
		...
		// TODO 自动生成方法存根
		String sql="";
		sql="select distinct stuff_num, realname, group_num, remark from v_user2role where id is not null and "+(String)hashMap.get("filter");
		Logger.debug(sql);
...
//			创建要显示的具体内容的题头
			...
			Label Stuff_mnum = new Label(1, 4, "用户工号",center);
			sheet.addCell(Stuff_mnum);
			
			Label Realname = new Label(2, 4, "用户姓名",center);
			sheet.addCell(Realname);
			
			Label Group_num = new Label(3, 4, "组别",center);
			sheet.addCell(Group_num);
			
			Label Remark = new Label(4, 4, "备注",center);
	    	...
	    	for(int i=0;i<pcArray.size();i++){
				...
				sheet.addCell(new Label(2,nRow,realname,center));
				sheet.addCell(new Label(3,nRow,group_num,center));
				sheet.addCell(new Label(4,nRow,remark,center));
				sheet.addCell(new Label(5,nRow,""));
				nRow++;
				k++;
	    	}
