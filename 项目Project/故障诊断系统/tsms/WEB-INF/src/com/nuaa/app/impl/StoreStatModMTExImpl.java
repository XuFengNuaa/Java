...
import com.nuaa.app.StoreStatModMTEx;
import com.nuaa.sys.util.DbUtil;
import com.nuaa.sys.util.IResultSetProcessor;
import com.nuaa.sys.util.Logger;

public class StoreStatModMTExImpl implements StoreStatModMTEx {

	public void StoreStatModMTType1Ex(HashMap hashMap, OutputStream os)
...
String sql="";
sql="select * from mt_type_base where "+filter+" order by "+order+"";
Logger.debug(sql);
...
//	创建标题，导出时间及查询条件
	...
	Label fname = new Label(0, 0, "标题：测量类工具当前库存统计",seat);
	sheet.addCell(fname);
	...
	Label nc_count_in1= new Label(5, 4, "新工具在库量",center);
	sheet.addCell(nc_count_in1);
	Label uc_count_in1 = new Label(6, 4, "旧工具在库量",center);
	...
	for(int i=0;i<pcArray.size();i++){
		...
		if(((HashMap)pcArray.get(i)).get("umt_count_in")!=null){
		 ...
		 int umtcount_in = Integer.parseInt(nmt_count_in);
...
public void StoreStatModMTType2Ex(HashMap hashMap, OutputStream os)
throws WriteException, IOException {
//TODO 自动生成方法存根
System.out.println("filter="+(String)hashMap.get("filter"));
String filter=(String)hashMap.get("filter");
...
Label fname = new Label(0, 0, "标题：测量类工具借用频率统计",seat);
sheet.addCell(fname);
...
public void StoreStatModMTIndi1Ex(HashMap hashMap, OutputStream os)
throws WriteException, IOException {
//TODO 自动生成方法存根
System.out.println("filter="+(String)hashMap.get("filter"));
String filter=(String)hashMap.get("filter");
...
String result=(String)hashMap.get("result");
String sql="";
sql="select * from mt_entry where "+filter+" order by "+order+"";
Logger.debug(sql);
final ArrayList pcArray=new ArrayList();
try{
DbUtil.execute(sql,new IResultSetProcessor() {
...
//创建标题，导出时间及查询条件
sheet.mergeCells(0, 0, 8, 0);
Label fname = new Label(0, 0, "标题：测量类工具入库历史记录",seat);
sheet.addCell(fname);
sheet.mergeCells(0, 1, 8, 1);
Label ftime = new Label(0, 1, "导出日期："+CurDate,wordsize);
sheet.addCell(ftime);
...
public void StoreStatModMTIndi2Ex(HashMap hashMap, OutputStream os)
throws WriteException, IOException {
//TODO 自动生成方法存根
System.out.println("filter="+(String)hashMap.get("filter"));
String filter=(String)hashMap.get("filter");
...
String result=(String)hashMap.get("result");
String sql="";
sql="select * from mt_br_now where "+filter+" order by "+order+"";
Logger.debug(sql);
final ArrayList pcArray=new ArrayList();
try{
DbUtil.execute(sql,new IResultSetProcessor() {
...
//创建标题，导出时间及查询条件
sheet.mergeCells(0, 0, 7, 0);
Label fname = new Label(0, 0, "标题：测量类工具当前借出记录",seat);
sheet.addCell(fname);
sheet.mergeCells(0, 1, 7, 1);
Label ftime = new Label(0, 1, "导出日期："+CurDate,wordsize);
sheet.addCell(ftime);
...
public void StoreStatModMTIndi3Ex(HashMap hashMap, OutputStream os)
throws WriteException, IOException {
//TODO 自动生成方法存根
System.out.println("filter="+(String)hashMap.get("filter"));
String filter=(String)hashMap.get("filter");
...
String result=(String)hashMap.get("result");
String sql="";
sql="select * from mt_br_record where "+filter+" order by "+order+"";
Logger.debug(sql);
final ArrayList pcArray=new ArrayList();
try{
DbUtil.execute(sql,new IResultSetProcessor() {
	...
	try{
		...
		while(rs.next()){
			...
			if(rs.getString("state").equals("5")){
				paHashMap.put("status", "工具归还");}
			else if (rs.getString("state").equals("4")) {
				paHashMap.put("status", "工具借用");}
...
Label fname = new Label(0, 0, "标题：测量类工具借还历史记录",seat);
sheet.addCell(fname);
