...
public class MaintainModImpl implements MatainMod{
	...
	public boolean addFile(HashMap addMap) {
		...
		System.out.println(pa[1]);
		 sql[0] = "insert into t_matain (ID,dalei_sim,xiaolei_sim,machine_sim,material_sim,daihao_sim,mingcheng_sim,tuhao_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		 try {
	...
	public JSONObject getQueryFile(HashMap hashmap) {
		...
		String sql="";
		sql="select  count(*) as ct  from t_matain ";
		...
		String sql1 = "select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				...
				"(select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				"t_matain where "+filter+" order by "+order+") where rownum<="+(start+limit)+" minus select  " +
				...
				"(select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				"t_matain where "+filter+" order by "+order+" ) where rownum<="+start+") order by "+order; 
		try {
	...
	public 	String getFile2(String dir,String tuhao){
		String bb="";
		System.out.println("hhhh");
		File fii=new File(dir+"\\web\\bom2\\"+tuhao+"\\");
		System.out.println(dir+"\\web\\bom2\\"+tuhao+"\\");
		if(!fii.exists()) {
			System.out.println("chucuo");
			   return bb; 
			}
	...
	public HashMap uploadFile2(HashMap fileHmap) {
    	...
    	System.out.println("333333"+id);
    	String tempPath = "\\web\\bom2\\"+id+"\\"; // 用于存放临时文件的目录
    	//String imgdir = "\\image\\";
		String trace="";
		String filename="";
		String fileid="";
	...
	public String delPart(HashMap hashMap) {
		...
		sql[0] = "delete from t_matain where id in ('',"+id+")";
		String sql2 = "select * from t_matain where id in ('',"+id+")";
		try {
