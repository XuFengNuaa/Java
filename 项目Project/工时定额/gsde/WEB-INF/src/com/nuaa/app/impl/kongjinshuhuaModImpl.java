...
import com.nuaa.app.kongjinshuhuaMod;
...
public class kongjinshuhuaModImpl implements kongjinshuhuaMod{
			...
			Logger.debug(sql);
			sql="select count(*) as ct from T_KONGJINSHUHUA where leibie='"+leibie+"'";
			...
			try {
				if ("0".equals(count[0])){	
					...
					sqls[0]= "insert into T_KONGJINSHUHUA (id,leibie,mj1,mj2,mj3,mj4,mj5)values(?,?,?,?,?,?,?)";
		...
		public JSONObject getQueryCompany(HashMap queryCompanyMap) {
			...
			String sql="";
			sql="select  count(*) as ct  from  T_KONGJINSHUHUA where "+(String)queryHashMap.get("filter");
			final String [] count=new String[1];
			count[0] = "";
			try {
				DbUtil.execute(sql,new IResultSetProcessor() {
			...
			sql="select * from (select * from  (select * from T_KONGJINSHUHUA where "+filter+" order by "+order+") where rownum<="+(start+limit)+" minus select  *  from  (select * from T_KONGJINSHUHUA where "+filter+" order by "+order+" ) where rownum<="+start+") order by "+order; 
			Logger.debug(sql);
			try {			
				DbUtil.execute(sql,new IResultSetProcessor() {
					public void process(ResultSet rs) throws SQLException {					
		...
		public JSONObject getQueryCompanyAll() {
			...
			final JSONArray jsonArray=new JSONArray();
			String sql="";
			sql="select * from T_KONGJINSHUHUA ";
		...
		public String delCompany(HashMap delCompanyHMap) {
			...
			try {
				sqls[0]="delete from T_KONGJINSHUHUA where id in('',"+id+")";
		...
		public String editCompany(HashMap editCompanyHMap) {
			...
			String sql="";
			sql="select count(*) as ct from T_KONGJINSHUHUA where leibie='"+leibie+"' and mj1 ='"+mj1+"' and mj2 ='"+mj2+"' and mj3 ='"+mj3+"' and mj4 ='"+mj4+"' and mj5 ='"+mj5+"' and id !='"+id+"'";
			final String[] count=new String[1];
			try {			
				DbUtil.execute(sql,new IResultSetProcessor(){
					public void process(ResultSet rs) throws SQLException{
		   ...
		   if ("0".equals(count[0])){							
			String[] sqls = new String[1];	
			sqls[0] = "update T_KONGJINSHUHUA set leibie=?,mj1=?,mj2=?,mj3=?,mj4=?,mj5=? where id='"+id+"'";
		...
		public JSONObject viewCompany(HashMap viewCompanyMap) {
			...
			String sql="";
			sql="select * from T_KONGJINSHUHUA where id='"+id+"'"; 
			Logger.debug(sql);
			try {			
				DbUtil.execute(sql,new IResultSetProcessor() {
					public void process(ResultSet rs) throws SQLException {					
