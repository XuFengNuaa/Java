...
public class PartProcessModImpl implements PartProcessMod{
	@Override
	public boolean addFile(HashMap addMap) {
		...
		System.out.println(pa[1]);
		 sql[0] = "insert into t_millsim (ID,dalei_sim,xiaolei_sim,machine_sim,material_sim,daihao_sim,mingcheng_sim,tuhao_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		 try {
	...
	public JSONObject getQueryFile(HashMap hashmap) {
		...
		String sql="";
		sql="select  count(*) as ct  from T_MILLSIM ";
		...
		String sql1 = "select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				...
				"(select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				"T_MILLSIM where "+filter+" order by "+order+") where rownum<="+(start+limit)+" minus select  " +
				...
				"(select id,dalei_sim,xiaolei_sim,daihao_sim,mingcheng_sim,tuhao_sim,material_sim,machine_sim,maopilong_sim,maopiwidth_sim,maopihigh_sim,qxtj_sim,qxmj_sim,zxd_sim from " +
				"T_MILLSIM where "+filter+" order by "+order+" ) where rownum<="+start+") order by "+order; 
		try {
	...
	public String delPart(HashMap hashMap) {
		...
		sql[0] = "delete from t_millsim where id in ('',"+id+")";
		String sql2 = "select * from t_millsim where id in ('',"+id+")";
		try {
