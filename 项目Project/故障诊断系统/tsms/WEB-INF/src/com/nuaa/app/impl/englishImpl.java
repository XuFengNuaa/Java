...
public class englishImpl implements english {
	public HashMap uploadFile1(HashMap fileHmap) {
    	...
    	String tempPath = "\\web\\english\\"+id+"\\"; // 用于存放临时文件的目录
    	//String imgdir = "\\image\\";
		String trace="";
		String filename="";
		String fileid="";
	...
	public 	String getFile1(String dir,String tuhao){
		...
		File fii=new File(dir+"\\web\\english\\"+tuhao+"\\");
		System.out.println(dir+"\\web\\english\\"+tuhao+"\\");
		if(!fii.exists()) {
		...
		for(int i=0;i<files.length;i++){
			...
			if(typ.equals("jpg")||typ.equals("png")){
				bb += "<a href=\"..\\..\\..\\..\\..\\tsms\\web\\english\\"+tuhao+"\\" +
			...
			if(typ.equals("docx")||typ.equals("doc")||typ.equals("pdf")||typ.equals("txt")){
				...
				//aa+=dir+"\\web\\english\\"+tuhao+"\\"+ files[i].getName() ;
			}
		}
	...
	public JSONObject getQueryApprove1(HashMap queryMap) {
		...
		String sql="";
		sql="select count(*) as ct from t_english1 where "+filter;
		Logger.debug(sql);
		...
		sql="select * from  t_english1  where "+filter;  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
	...
	public String delPart1(HashMap hashMap) {
		...
		sql[0] = "delete from t_english1 where id in ('',"+id+")";
		String sql2 = "select * from t_english1 where id in ('',"+id+")";
		try {
	...
	public boolean addFile1(HashMap addMap) {
		...
		System.out.println(pa[1]);
		 sql[0] = "insert into t_english1 (ID,scr,tpmc,time)values(?,?,?,to_date(?,'yyyy/mm/dd'))";
		 try {
			DbUtil.executeBatchs(sql,new IArrayPreparedStatementProcessor(){
	...
	public JSONObject getQueryApprove(HashMap queryMap) {
		...
		final JSONArray jsonArray=new JSONArray();
		String sql="";
		sql="select count(*) as ct from t_english where "+filter;
		Logger.debug(sql);
		final String [] count=new String[1];
		count[0] = "";
		try {
		...
		sql="select * from  t_english  where "+filter;  
		Logger.debug(sql);
		try {			
			DbUtil.execute(sql,new IResultSetProcessor() {
				public void process(ResultSet rs) throws SQLException {					
	...
	public String delPart(HashMap hashMap) {
		...
		String []sqls=new String[1];
		final String[] sql = new String[1];
		sql[0] = "delete from t_english where id in ('',"+id+")";
		String sql2 = "select * from t_english where id in ('',"+id+")";
		try {
			DbUtil.execute(sql2,new IResultSetProcessor(){
				public void process(ResultSet rs) throws SQLException {
					// TODO Auto-generated method stub
	...
	public boolean addFile(HashMap addMap) {
		...
		String[] sql = new String[1];
		System.out.println(pa[1]);
		 sql[0] = "insert into t_english (ID,scr,tpmc,time)values(?,?,?,to_date(?,'yyyy/mm/dd'))";
		 try {
			DbUtil.executeBatchs(sql,new IArrayPreparedStatementProcessor(){
				public void process(PreparedStatement[] pstmts) throws SQLException {
					// TODO Auto-generated method stub
