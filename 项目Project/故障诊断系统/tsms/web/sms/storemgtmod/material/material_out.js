Ext.onReady(function(){
	...
	var hpObj={url:'logicmaterial_out.jsp',method: 'POST',extraParams:{}};
	...
	function customQueryFn(){
	    ...
	    var partdalei= Ext.data.Record.create([
		 ...
		 var storedalei = new Ext.data.Store ({
		     proxy: new Ext.data.HttpProxy({url:"logicmaterial_delete.jsp?type=QUERYDALEI"}), // 数据源
		     reader: new Ext.data.JsonReader({
		         id:'id',
			     root:"filedata"
			       },partdalei),
		...
		var storexiaolei = new Ext.data.Store ({
	     proxy: new Ext.data.HttpProxy({url:"logicmaterial_delete.jsp?type=QUERYXIAOLEI"}), // 数据源
	     reader: new Ext.data.JsonReader({
	         id:'id',
		     root:"filedata"
		       },partxiaolei),
	  ...
	  // 定义添加窗口材料
		 ...
		 var materialCom = new Ext.data.Store ({
		     proxy: new Ext.data.HttpProxy({url:"logicmaterial_delete.jsp?type=QUERYMATERIAL"}), // 数据源
		     reader: new Ext.data.JsonReader({
		         id:'id',
			     root:"filedata"
			       },material_com),
		 ...
		 // 定义机床型号
			 ...
			 var machineCom = new Ext.data.Store ({
			     proxy: new Ext.data.HttpProxy({url:"logicmaterial_delete.jsp?type=QUERYMACHINE"}), // 数据源
			     reader: new Ext.data.JsonReader({
			         id:'id',
				     root:"filedata"
				       },machine_com),
		 ...
		 var button1 = null;
			...
			if (!dialog1) {
				dialog1 = new Ext.ux.UploadDialog.Dialog({
				  url: 'logicmaterial_out.jsp?type=uploadimg&ljth='+ljth, 
				  reset_on_hide: false,
				  modal:true,
				  closable : false,   
		          collapsible : false,   
