Ext.onReady(function(){
    ...
    //调用数据库信息
	var hqObj={url:'logic_checkmod.jsp',method: 'POST',extraParams:{type:'QUERYM',params:{start:start,limit:limit},filter:filter}};
  ...
  //刷新函数   
			...
			Ext.MessageBox.confirm('提示','是否确定批准？？？',function(btn) {
				...
				Ext.showProgressDialog();
					...
					Ext.Ajax.request({
						 	...
						 	method:'post',
					 		params:{type:'APPLYM',entry_nums:entry_nums,length:length},
			...
			Ext.MessageBox.confirm('提示','是否确定拒绝？？？',function(btn) {
				...
				Ext.showProgressDialog();
					...
					Ext.Ajax.request({
						 	url:'logic_checkmod.jsp',
						 	method:'post',
					 		params:{type:'MRefuse',entry_nums:entry_nums,length:length},
							success:function(result){
								if (result.responseText.indexOf("success") != -1){
									Ext.hideProgressDialog();	
									alert("提示：拒绝成功!");
	...
	function outFn(){
		if(ds.getCount()==0){
			...
			var url = 'http://localhost:8080/tsms/web/sms/ldappmod/check/mentry_excel.jsp';
    		location.href='mentry_excel.jsp?url='+url;
    	}
	}
