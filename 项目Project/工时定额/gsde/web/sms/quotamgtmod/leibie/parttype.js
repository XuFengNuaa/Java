...
Ext.onReady(function(){
    ...
    //定义查询结果页面
	var hpObj={url:'logicparttype.jsp',method: 'POST',extraParams:{}};
   ...
   //修改、添加弹出panel
     var formpanel = new Ext.FormPanel({
        ...
        items: [
                {xtype: 'textfield',fieldLabel:"大&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;类",width:210,name:"dalei",id:"dalei",allowBlank:false,blankText:"大类不能为空，请填写!"},
                {xtype: 'textfield',fieldLabel:"小&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;类",width:210,name:"xiaolei",id:"xiaolei",allowBlank:false,blankText:"小类不能为空，请填写!"}
               ]
		    });
	//定义弹出窗口
    var win = new Ext.Window({
                ...
                buttons: [{
		                ...
		                handler:function(){
							...
							var dalei=Ext.get("dalei").dom.value;
							var xiaolei=Ext.get("xiaolei").dom.value;
	...
	function editwinFn(){	
		...
		if(record.length==0){
        	...
        	alert('每次只能修改一条零件类型信息!');
            return;
        }else{ if(!win.isVisible()){
                win.setTitle("<p align='center'>零件类型信息修改</p>");
				win.show();//修改窗口显示
				...
				initializeFn();
				Ext.getCmp("dalei").getEl().dom.readOnly  = false;
				Ext.getCmp("xiaolei").getEl().dom.readOnly  = false;
	...
	function successFn(result,request){
		...
		win.findById("dalei").setValue(objtp.dalei);
		win.findById("xiaolei").setValue(objtp.xiaolei);
		...
		if(record.length!=0) {
       	...
       	function addFn(){
       	  if(!win.isVisible()){
				...
				win.setTitle("<p align='center'>添加零件类型信息</p>")
				win.show();//添加窗口显示
	 ...
	 //明细窗口显示
	  function viewFn(){
			...
			if(record.length==0){
                ...
                win.setTitle("<p align='center'>零件类型信息明细</p>")
				win.show();//明细窗口显示
				...
				initializeFn();
				Ext.getCmp("dalei").getEl().dom.readOnly  = true;
				Ext.getCmp("xiaolei").getEl().dom.readOnly  = true;
				}
			} 
	    } 
