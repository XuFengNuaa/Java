Ext.onReady(function(){
	...
	var hpObj={url:'logicpartprocess.jsp',method: 'POST',extraParams:{}};
	...
	var grid = new Ext.grid.GridPanel({
        ...
        columns: [sm,
                   {header: "零件大类",width: 80,align:'center',dataIndex: "dalei_sim",sortable: true},
                   {header: "零件小类",width:100,align:'center',dataIndex: "xiaolei_sim",sortable: true},
                   ...
                   {header: "毛坯长度",width:80,align:'center',dataIndex: "maopilong_sim",sortable: true},
		...
		items:[search,
				...
				tbar:new Ext.Toolbar({ 
						items:[toolone,{id:'add-btn',text:"增加实例",handler:addFn,iconCls:"add"},
							...
							toolfour,{id:'pass-btn',text:"上传零件图片",handler: imgFn,iconCls:"add"},
							//toolfive,{id:'img-btn',text:"修改零件图片",handler: imgEditFn,iconCls:"update"},
							toolsix,{id:'img-btn',text:"查看零件图片",handler: viewImgFn,iconCls:"detail"}]
		        }),
	  ...
	  // 定义添加窗口材料
		...
		var inform=new Ext.FormPanel({
		    ...
		    items: [{
		            ...
		            items:[{
		                ...
		                labelAlign:"right",
		                items: [
		                   {xtype:'textfield',fieldLabel: '毛坯长度',name: 'maopilong',width:150,id: 'maopilong'},
		                   {xtype:'textfield',fieldLabel: '毛坯宽度',name: 'maopikuan',width:150,id: 'maopikuan'},
		                   {xtype:'textfield',fieldLabel: '毛坯高度',name: 'maopigao',width:150,id: 'maopigao'},
		                   {xtype:'textfield',fieldLabel: '切削体积',name: 'qxtj',width:150,id: 'qxtj'},
		                   {xtype:'textfield',fieldLabel: '切削表面积',name: 'qxmj',width:150,id: 'qxmj'},
		...
		function savebtn(){
        	if(Ext.get("cpdh").dom.value.trim()==""){
     	        ...
     	        return;
        }else if(Ext.get("zxd").dom.value.trim()==""){
     	        Ext.MessageBox.alert('提示',"实例置信度为必填项");
     	        return;
     	        ...
     	        return;
        }else if(ComboBoxMaterial.getValue()==""){
 	        Ext.MessageBox.alert('提示',"材料为必填项");
 	        return;
        }else if(ComboBoxmachine.getValue()==""){
 	        Ext.MessageBox.alert('提示',"设备为必填项");
 	        return;
        }else if(Ext.get("maopilong").dom.value.trim()==""){
     	        Ext.MessageBox.alert('提示',"毛坯长度为必填项");
     	        return;
        }else if(Ext.get("maopikuan").dom.value.trim()==""){
 	        Ext.MessageBox.alert('提示',"毛坯长度为必填项");
 	        return;
        }else if(Ext.get("maopigao").dom.value.trim()==""){
 	        Ext.MessageBox.alert('提示',"毛坯长度为必填项");
 	        return;
        }else if(Ext.get("qxtj").dom.value.trim()==""){
 	        Ext.MessageBox.alert('提示',"切削体积为必填项");
 	        return;
        }else if(Ext.get("qxmj").dom.value.trim()==""){
     	        Ext.MessageBox.alert('提示',"切削表面积为必填项");
     	        return;
        }
		 else{
			 insertSaveFn();}	
		...
		function imgFn(){
		 ...
		 var button1 = null;
			function getDialog1(){
				var record = sm.getSelections();
				var ljth = record[0].get("ljth");
			if (!dialog1) {
				dialog1 = new Ext.ux.UploadDialog.Dialog({
				  url: 'logicpartprocess.jsp?type=uploadimg&ljth='+ljth, 
				  reset_on_hide: false,
				  modal:true,
				  closable : false,   
		          collapsible : false,   
	...
	function addsaveFn(){
		...
		function insertSaveFn(){
				...
				var machine_sim = ComboBoxmachine.getValue();
				var material_sim = ComboBoxMaterial.getValue();
